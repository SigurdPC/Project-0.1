...
const DailyEvents = () => {
  ...
  const { checkDuplicate, isFutureDate } = useValidation<DailyEvent>(
    data,
  ...
  const handleAdd = async (newData: DailyEvent) => {
    ...
    try {
      ...
      if (isFutureDate(dataToSend.date)) {
        showSnackbar('Cannot add record with future date.', 'error');
        setLoading(false);
        return false;
      }
  ...
  const handleEdit = async (id: string, updatedData: DailyEvent) => {
    ...
    try {
      ...
      // Проверка на дату в будущем
      if (isFutureDate(dataToSend.date)) {
        showSnackbar('Cannot update record with future date.', 'error');
        setLoading(false);
        return false;
      }
